...
//Ellen�rzi, hogy egy pixel ne legyen kisebb 0-n�l �s ne legyen nagyobb a maximum megengedett �rt�kn�l
        ...
        for (int i = 0; i < img->height; i++)
        {
            for (int x = 0; x < img->width; x++)
            {
                ...
                for (int z = 0; z < radius * 2 + 1; z++)
                {
                    ...
                    {
                        //Fent van tulcsordul�s
                        if (i - radius + z < 0) {
                            //Balra van t�lcsordul�s
                            if (x - radius + a < 0) {
                                sum[0] += img->pixels[0][0].r;
                                sum[1] += img->pixels[0][0].g;
                                sum[2] += img->pixels[0][0].b;
                            }
                            //Jobbra van t�lcsordul�s
